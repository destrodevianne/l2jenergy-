...
public class NpcViewMod implements IBypassHandler
{
	...
	public boolean useBypass(String command, L2PcInstance activeChar, L2Character bypassOrigin)
	{
		...
		if (!st.hasMoreTokens())
		{
			LOG.warn("Bypass[NpcViewMod] used without enough parameters.");
			return false;
		}
		...
		switch (actualCommand.toLowerCase())
		{
			...
			case "droplist":
			{
				...
				{
					LOG.warn("Bypass[NpcViewMod] unknown drop list scope: {}", dropListScopeString);
					return false;
				}
				break;
			}
	...
	public static void sendNpcDropList(L2PcInstance activeChar, L2Npc npc, DropListScope dropListScope, int page)
	{
			...
			else if (dropItem instanceof GroupedGeneralDropItem)
			{
				...
				else
				{
					...
					for (GeneralDropItem generalDropItem : normalized.getItems())
					{
						...
						MinMax minMax = getPreciseMinMax(normalized.getChance(), generalDropItem.getMin(npc, npc), generalDropItem.getMax(npc, npc), generalDropItem.isPreciseCalculated());
		...
		if (html == null)
		{
			LOG.warn("{}: The html file data/html/mods/NpcView/DropList.htm could not be found.", NpcViewMod.class.getSimpleName());
			return;
		}
	...
	private static void addGeneralDropItem(L2PcInstance activeChar, L2Npc npc, final DecimalFormat amountFormat, final DecimalFormat chanceFormat, final StringBuilder sb, final GeneralDropItem dropItem)
	{
		...
		sb.append("<td width=247 align=center>");
		MinMax minMax = getPreciseMinMax(dropItem.getChance(npc, activeChar), dropItem.getMin(npc, npc), dropItem.getMax(npc, npc), dropItem.isPreciseCalculated());
