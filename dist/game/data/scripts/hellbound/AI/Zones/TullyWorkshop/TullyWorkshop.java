...
public final class TullyWorkshop extends AbstractNpcAI
{
	...
	public final String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		else if (npcId == DWARVEN_GHOST)
		{
			...
			{
				broadcastNpcSay(npc, ChatType.NPC_ALL, NpcStringId.HA_HA_YOU_WERE_SO_AFRAID_OF_DEATH_LET_ME_SEE_IF_YOU_FIND_ME_IN_TIME_MAYBE_YOU_CAN_FIND_A_WAY);
				npc.deleteMe();
				return null;
			}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
			...
			_countdown = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(() ->
			{
				...
				if (countdownTime > 60000)
				{
					if ((countdownTime % 60000) == 0)
					{
						...
						{
							broadcastNpcSay(_npc, ChatType.NPC_SHOUT, NpcStringId.S1_MINUTES_REMAINING, Integer.toString((countdownTime / 60000)));
						}
					}
				}
				...
				else
				{
					if ((_npc != null) && (_npc.getId() == INGENIOUS_CONTRAPTION))
					{
						broadcastNpcSay(_npc, ChatType.NPC_SHOUT, NpcStringId.S1_SECONDS_REMAINING, Integer.toString((countdownTime / 1000)));
					}
				}
			}, 60000, 10000);
			broadcastNpcSay(postMortemSpawn.get(0), ChatType.NPC_SHOUT, NpcStringId.DETONATOR_INITIALIZATION_TIME_S1_MINUTES_FROM_NOW, Integer.toString((countdownTime / 60000)));
		}
		else if ((npcId == TIMETWISTER_GOLEM) && (_countdown != null))
		{
			if (getRandom(1000) >= 700)
			{
				broadcastNpcSay(npc, ChatType.NPC_ALL, NpcStringId.A_FATAL_ERROR_HAS_OCCURRED);
			...
			{
				broadcastNpcSay(npc, ChatType.NPC_ALL, NpcStringId.TIME_RIFT_DEVICE_ACTIVATION_SUCCESSFUL);
		...
		else if ((npcId >= SERVANT_FIRST) && (npcId <= SERVANT_LAST))
		{
			...
			{
				broadcastNpcSay(npc, ChatType.NPC_SHOUT, NpcStringId.S1_ILL_BE_BACK_DONT_GET_COMFORTABLE, killer.getName());
			}
		}
	...
	public String onSpellFinished(L2Npc npc, L2PcInstance player, Skill skill)
	{
		...
		{
			broadcastNpcSay(npc, ChatType.NPC_ALL, NpcStringId.S1_THANK_YOU_FOR_GIVING_ME_YOUR_LIFE, player.getName());
