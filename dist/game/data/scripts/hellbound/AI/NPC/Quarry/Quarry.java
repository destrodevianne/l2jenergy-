	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			...
			case "TIME_LIMIT":
			{
				...
				broadcastNpcSay(npc, ChatType.NPC_ALL, NpcStringId.HUN_HUNGRY);
				npc.doDie(npc);
				break;
			}
			case "DECAY":
				...
				if ((npc != null) && !npc.isDead())
				{
					if (npc.getTarget().isPlayer())
					{
						for (ItemChanceHolder item : DROP_LIST)
						{
							...
							{
								npc.dropItem((L2PcInstance) npc.getTarget(), item.getId(), (int) (item.getCount() * RatesConfig.RATE_QUEST_DROP));
								break;
							}
						}
					}
			...
			if (npc.getId() == SLAVE)
			{
				if (!npc.isDead() && !npc.isDecayed() && (npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_FOLLOW))
				{
					if (HellboundEngine.getInstance().getLevel() == 5)
					{
						...
						try
						{
							broadcastNpcSay(npc, ChatType.NPC_ALL, NpcStringId.THANK_YOU_FOR_THE_RESCUE_ITS_A_SMALL_GIFT);
						}
						catch (Exception e)
						{
							//
