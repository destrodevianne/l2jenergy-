...
public final class Amaskari extends AbstractNpcAI
{
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			broadcastNpcSay(npc, ChatType.NPC_ALL, AMASKARI_NPCSTRING_ID[2]);
		...
		else if (event.equalsIgnoreCase("onspawn_msg") && (npc != null) && !npc.isDead())
		{
			if (getRandom(100) > 20)
			{
				broadcastNpcSay(npc, ChatType.NPC_ALL, MINIONS_NPCSTRING_ID[2]);
			}
			else if (getRandom(100) > 40)
			{
				broadcastNpcSay(npc, ChatType.NPC_ALL, MINIONS_NPCSTRING_ID[3]);
			}
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon, Skill skill)
	{
		...
		{
			broadcastNpcSay(npc, ChatType.NPC_ALL, AMASKARI_NPCSTRING_ID[0]);
			for (L2MonsterInstance minion : ((L2MonsterInstance) npc).getMinionList().getSpawnedMinions())
			{
				...
				{
					broadcastNpcSay(minion, ChatType.NPC_ALL, MINIONS_NPCSTRING_ID[0]);
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		if (npc.getId() == AMASKARI_PRISONER)
		{
			...
			{
				broadcastNpcSay(master, ChatType.NPC_ALL, AMASKARI_NPCSTRING_ID[1]);
				...
				else
				{
					...
					{
						broadcastNpcSay(master, ChatType.NPC_ALL, AMASKARI_NPCSTRING_ID[3]);
		...
		else if (npc.getId() == AMASKARI)
		{
			for (L2MonsterInstance minion : ((L2MonsterInstance) npc).getMinionList().getSpawnedMinions())
			{
				if ((minion != null) && !minion.isDead())
				{
					...
					{
						broadcastNpcSay(minion, ChatType.NPC_ALL, MINIONS_NPCSTRING_ID[1]);
					}
