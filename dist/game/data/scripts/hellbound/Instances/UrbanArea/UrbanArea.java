...
public final class UrbanArea extends AbstractInstance
{
		...
		if ((tmpworld != null) && (tmpworld instanceof UrbanAreaWorld))
		{
			...
			if (npc.getId() == DOWNTOWN_NATIVE)
			{
				...
				else if (event.equalsIgnoreCase("break_chains"))
				{
					...
					{
						broadcastNpcSay(npc, ChatType.NPC_ALL, NATIVES_NPCSTRING_ID[0]);
						broadcastNpcSay(npc, ChatType.NPC_ALL, NATIVES_NPCSTRING_ID[2]);
					}
					else
					{
						...
						broadcastNpcSay(npc, ChatType.NPC_ALL, NATIVES_NPCSTRING_ID[0]);
						broadcastNpcSay(npc, ChatType.NPC_ALL, NATIVES_NPCSTRING_ID[1]);
	...
	public String onAggroRangeEnter(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if ((tmpworld != null) && (tmpworld instanceof UrbanAreaWorld))
		{
			...
			if (!npc.isBusy())
			{
				broadcastNpcSay(npc, ChatType.NPC_ALL, NPCSTRING_ID[0]);
				npc.setBusy(true);
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon, Skill skill)
	{
		...
		if ((tmpworld != null) && (tmpworld instanceof UrbanAreaWorld))
		{
			...
			if (!world.isAmaskariDead && !(npc.getBusyMessage().equalsIgnoreCase("atk") || npc.isBusy()))
			{
				...
				if (msgId >= 0)
				{
					broadcastNpcSay(npc, ChatType.NPC_ALL, NPCSTRING_ID[msgId], range);
				}
				npc.setBusy(true);
			...
			if ((_caller != null) && !_caller.isDead())
			{
				...
				if ((tmpworld != null) && (tmpworld instanceof UrbanAreaWorld))
				{
					...
					if ((world.spawnedAmaskari != null) && !world.spawnedAmaskari.isDead())
					{
						...
						world.spawnedAmaskari.broadcastPacket(new NpcSay(world.spawnedAmaskari.getObjectId(), ChatType.NPC_ALL, world.spawnedAmaskari.getId(), NpcStringId.ILL_MAKE_YOU_FEEL_SUFFERING_LIKE_A_FLAME_THAT_IS_NEVER_EXTINGUISHED));
					}
				}
			}
		}
