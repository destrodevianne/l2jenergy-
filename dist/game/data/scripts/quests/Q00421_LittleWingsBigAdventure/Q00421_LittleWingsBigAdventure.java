...
public final class Q00421_LittleWingsBigAdventure extends Quest
{
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon)
	{
		...
		if ((qs != null) && qs.isCond(2))
		{
			if (isSummon)
			{
				...
				if ((qs.getMemoState() % data.getMemoStateMod()) < data.getMemoStateValue())
				{
					if (attacker.getSummon().getControlObjectId() == qs.getInt("fluteObjectId"))
					{
						...
						else if (getRandom(100) < 2)
						{
							...
							{
								npc.broadcastPacket(new NpcSay(npc, ChatType.NPC_ALL, NpcStringId.GIVE_ME_A_FAIRY_LEAF));
								takeItems(attacker, FAIRY_LEAF, 1);
								qs.setMemoState(qs.getMemoState() + data.getMemoStateValue());
								qs.unset("hits");
				...
				else
				{
					...
					{
						case 0:
							npc.broadcastPacket(new NpcSay(npc, ChatType.NPC_ALL, NpcStringId.WHY_DO_YOU_BOTHER_ME_AGAIN));
							break;
						case 1:
							npc.broadcastPacket(new NpcSay(npc, ChatType.NPC_ALL, data.getMessage()));
							break;
						case 2:
							npc.broadcastPacket(new NpcSay(npc, ChatType.NPC_ALL, NpcStringId.LEAVE_NOW_BEFORE_YOU_INCUR_THE_WRATH_OF_THE_GUARDIAN_GHOST));
							break;
					}
				}
			}
	...
	private static final class NpcData
	{
		...
		private final int _memoStateMod;
		private final int _memoStateValue;
		private final int _minHits;
		 ...
		 * @return the _message
		 */
		public NpcStringId getMessage()
		{
			return _message;
		}
		
		/**
		 * @return the _memoStateMod
		 */
		public int getMemoStateMod()
		{
			return _memoStateMod;
		}
		
		/**
		 * @return the _memoStateValue
		 */
		public int getMemoStateValue()
		{
			return _memoStateValue;
		}
