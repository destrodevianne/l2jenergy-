 ...
 */
package custom.events.PcBangPoint;
...
public class PcBangPoint extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (POINTSSKILL.containsKey(event))
		{
			...
			{
				htmltext = "skill_nolevel.html";
			}
			else if (player.getPcCafePoints() >= POINTSSKILL.get(event)[2])
			{
				final int cost = player.getPcCafePoints() - (POINTSSKILL.get(event)[2]);
				player.decreasePcCafePoints(cost);
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_ARE_USING_S1_POINT).addLong(POINTSSKILL.get(event)[2]));
				...
				return "Individual_skill_info.html";
			}
			else
			{
				htmltext = "nopoint.html";
			}
		}
		else if (PETSKILL.containsKey(event))
		{
			...
			{
				final int cost = player.getPcCafePoints() - (PETSKILL.get(event)[2]);
				player.decreasePcCafePoints(cost);
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_ARE_USING_S1_POINT).addLong(PETSKILL.get(event)[2]));
				...
				return "Individual_pet_skill_info.html";
			}
			else
			{
				htmltext = "nopoint.html";
		...
		else if (TELEPORTERS.containsKey(event))
		{
			...
			{
				final int cost = player.getPcCafePoints() - (TELEPORTERS.get(event)[3]);
				player.decreasePcCafePoints(cost);
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_ARE_USING_S1_POINT).addLong(TELEPORTERS.get(event)[3]));
		...
		{
			htmltext = npc.getId() + "-tele.html";
		}
		else if (event.equalsIgnoreCase("wyvern"))
		{
			if (player.getPcCafePoints() >= 2500)
			{
				final int cost = player.getPcCafePoints() - (2500);
				player.decreasePcCafePoints(cost);
		...
		else if (event.equalsIgnoreCase("warrior"))
		{
			...
			{
				final int cost = player.getPcCafePoints() - (5600);
				player.decreasePcCafePoints(cost);
		...
		else if (event.equalsIgnoreCase("pet_warrior"))
		{
			...
			{
				final int cost = player.getPcCafePoints() - (4000);
				player.decreasePcCafePoints(cost);
		...
		else if (event.equalsIgnoreCase("mage"))
		{
			...
			{
				final int cost = player.getPcCafePoints() - (3000);
				player.decreasePcCafePoints(cost);
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_ARE_USING_S1_POINT).addLong(4300));
		...
		else if (event.equalsIgnoreCase("pet_mage"))
		{
			...
			{
				final int cost = player.getPcCafePoints() - (2100);
				player.decreasePcCafePoints(cost);
	...
	private PcBangPoint()
	{
		super(-1, PcBangPoint.class.getSimpleName(), "custom/events");
