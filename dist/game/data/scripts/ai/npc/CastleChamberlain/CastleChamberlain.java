	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (st.nextToken())
		{
			...
			case "manor":
			{
				...
				{
					player.sendMessage(MessagesData.getInstance().getMessage(player, "castle_manor_deactivated"));
				}
				break;
			}
			case "products":
		...
		if (isOwner(player, npc))
		{
			...
			switch (evt.getRequest())
			{
				...
				case 8: // Crop settings
					...
					break;
				default:
					LOG.warn("{}: Player {} ({}) send unknown request id {}!", getClass().getSimpleName(), player.getName(), player.getObjectId(), evt.getRequest());
			}
		}
	}
