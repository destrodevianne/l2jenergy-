...
public class TopicBBSManager extends BaseBBSManager
{
	...
	public void parsewrite(String ar1, String ar2, String ar3, String ar4, String ar5, L2PcInstance activeChar)
	{
		if (ar1.equals("crea"))
		{
			...
			DAOFactory.getInstance().getForumDAO().findById(forum);
			
			final int id = TopicBBSManager.getInstance().getMaxId(forum) + 1;
			final long date = Calendar.getInstance().getTimeInMillis();
			final Topic topic = new Topic(id, Integer.parseInt(ar2), ar5, date, activeChar.getName(), activeChar.getObjectId(), TopicType.MEMO, 0);
		...
		else if (ar1.equals("del"))
		{
			...
			final List<Post> posts = PostBBSManager.getInstance().getGPosttByTopic(topic);
			if (!posts.isEmpty())
			{
				PostBBSManager.getInstance().delPostByTopic(topic);
				DAOFactory.getInstance().getPostDAO().delete(topic);
			}
			
			DAOFactory.getInstance().getTopicDAO().delete(topic, forum);
	...
	public void parsecmd(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("_bbstopics;del"))
		{
			...
			else
			{
				...
				{
					final List<Post> p = PostBBSManager.getInstance().getGPosttByTopic(t);
					if (p != null)
					{
						PostBBSManager.getInstance().delPostByTopic(t);
						DAOFactory.getInstance().getPostDAO().delete(t);
					}
					
					DAOFactory.getInstance().getTopicDAO().delete(t, f);
