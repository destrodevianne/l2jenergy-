	...
	public void runImpl()
	{
		...
		for (Entry entry : list.getEntries())
		{
			if (entry.getEntryId() == _entryId)
			{
				...
				{
					LOG.warn("Character: {} is trying to set amount > 1 on non-stackable multisell, id: {}: {}", player.getName(), _listId, _entryId);
					player.setMultiSell(null);
					return;
				}
				...
				for (Ingredient e : ingredientsList)
				{
					...
					else
					{
						...
						final long required = ((CharacterConfig.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient()) ? (e.getItemCount() * _amount) : e.getItemCount());
				...
				for (Ingredient e : entry.getIngredients())
				{
					...
					else
					{
						...
						{ // this is a cheat, transaction will be aborted and if any items already taken will not be returned back to inventory!
							LOG.warn("Character: {} is trying to cheat in multisell, id: {}: {}", player.getName(), _listId, _entryId);
							player.setMultiSell(null);
							return;
						}
						...
						if (CharacterConfig.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient())
						{
