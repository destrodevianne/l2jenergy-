		...
		if (_activeRoutes.containsKey(npc.getObjectId()))
		{
			...
			if ((walk.getCurrentNodeId() >= 0) && (walk.getCurrentNodeId() < walk.getRoute().getNodesCount()))
			{
				...
				if (npc.isInsideRadius(node, 10, false, false))
				{
					...
					{
						Broadcast.toKnownPlayers(npc, new NpcSay(npc, ChatType.NPC_ALL, node.getNpcString()));
					}
					else if (!node.getChatText().isEmpty())
					{
						Broadcast.toKnownPlayers(npc, new NpcSay(npc, ChatType.NPC_ALL, node.getChatText()));
					}
