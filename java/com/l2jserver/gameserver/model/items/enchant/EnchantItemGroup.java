...
public final class EnchantItemGroup
{
	...
	public double getChance(int index)
	{
		if (!_chances.isEmpty())
		{
			...
			LOG.warn("{}: Couldn't match proper chance for item group: {}", getClass().getSimpleName(), _name, new IllegalStateException());
			return _chances.get(_chances.size() - 1).getChance();
		}
		LOG.warn("{}: item group: {} doesn't have any chances!", getClass().getSimpleName(), _name);
		return -1;
	}
}
